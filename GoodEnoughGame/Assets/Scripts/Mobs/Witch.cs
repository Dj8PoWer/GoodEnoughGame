...
public class Witch : MonoBehaviour, IPunObservable
{
    ...
    void Update()
    {
        ...
        else if (Vector2.Distance(transform.position, target.transform.position) < back && fleeTime > 0)
        {
            transform.position = Vector2.MoveTowards(transform.position, target.transform.position, -speed * Time.deltaTime);
    ...
    {
        StartCoroutine(Attack(pos, target));
    ...
    IEnumerator Attack(Vector3 pos, Vector2 target)
    {
